...
class Session:
    ...
    def send_request(self):
        event_type = self.protocol._url.decode("utf-8")
        data = self.protocol._full_body
        corr_id = uuid.uuid4()
        response = requests.post("http://localhost:7000/serverless/instance",
                                    headers={"Content-Type": "application/json"},
                                    data=json.dumps({"input_data": json.dumps({"corr_id": str(corr_id), "body": json.loads(data), "headers": self.protocol._headers},
                                                                                default=lambda x: x.decode("utf-8")),
                                                    "event_type": event_type}))
        response_ids = response.json().get("instance_ids")
        return corr_id, response_ids[0] if len(response_ids) > 0 else None
